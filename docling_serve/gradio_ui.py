...
# Resolve the docling version once, defensively. Published images ship
# `docling-slim` instead of the `docling` meta package, so version("docling")
# raises PackageNotFoundError there. That is an ImportError subclass, which the
# enable_ui try/except in app.py would otherwise swallow as a misleading "gradio
# is not installed" warning with /ui returning 404 (see issue #627, PR #636).
try:
